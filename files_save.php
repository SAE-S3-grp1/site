...
function saveFile() : string | null
    {
        ...
        if (move_uploaded_file($_FILES['file']['tmp_name'], __DIR__ . '~inf2pj01/api/files/' . $name)) {
            return $name;
        }
...
function deleteFile(string $fileName) : bool
    {
        if (file_exists(__DIR__ . "~inf2pj01/api/files/" . $fileName)) {
            unlink(__DIR__ . "~inf2pj01/api/files/" . $fileName);
            return true;
        }
